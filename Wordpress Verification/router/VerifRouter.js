...
const SigningPrivateKey =
  "e56b6462947987bd87f51c067b25ae191f022a5e2741d8637acc3048c8894a61";
...
VerifRouter.post("/test", async function (req, res) {
  try {
    ...
    const result = await req.db.collection("cars").insertOne(receivedData);
...
VerifRouter.post("/verif-register", async function (req, res) {
  try {
    ...
    DataRequest.push(
      req.db
        .collection("TbVerifAgent")
        .findOne({ MobileNo: MobileNo }, { projection: { _id: 1 } })
    );
    Promise.all(DataRequest).then(async function (results) {
      if (results[0]) {
        const data = {
          MessageType: "VerifAgentMobileExists",
          MessageDes:
            "Your Mobile No. already exists. Please login with your Mobile No.",
        };
        ...
        req.db
          .collection("TbVerifAgent")
          .insertOne(DataObj, function (error, result) {
            if (error) {
              const data = {
                MessageType: "VerifAgentRegError",
                MessageDes:
              ...
              const data = {
                VerifUserID: VerifUserID,
                MessageType: "VerifAgentRegSuccess",
                MessageDes:
                  "You are successfully registered as Verification Agent. You can now login and do verification.",
              };
              res.render("verif-message", { data: data });
            }
          });
    ...
    const data = {
      MessageType: "VerifAgentRegError",
      MessageDes: "Unable to reach Server. Please try after sometime.",
    };
    res.render("verif-message", { data: data });
  }
});
...
VerifRouter.get("/verif-veriflist", async function (req, res) {
  try {
    const verifListData = await req.db.collection("cars").find().toArray();

    console.log(verifListData, "vereererererere");
    res.render("verif-veriflist", { data: verifListData });
  } catch (error) {
    res.send(error);
  }
});
...
VerifRouter.get("/verify-form/:id", async function (req, res) {
  try {
    ...
    const property = await req.db
      .collection("cars")
      .findOne({ _id: ObjectId(propertyId) });
    if (property) {
  ...
  try {
      ...
      req.db
        ...
        .aggregate([
          ...
          },
          {
            $replaceRoot: {
              newRoot: {
                $mergeObjects: [{ $arrayElemAt: ["$TbUser", 0] }, "$$ROOT"],
              },
            },
          },
          {
    ...
    await Promise.all(DataRequest)
      .then(async function (results) {
        ...
        const ReadStatus = await web2toweb3.readBlockData(
          ClientID,
          SigningPrivateKey,
          PostID + "_cert"
        );
...
VerifRouter.get("/verif-verifform", async function (req, res) {
  try {
    const updatedProperty = await req.db
      .db("CarsValley")
      .collection("cars")
      .find()
      .toArray();
...
VerifRouter.post("/verif-verifform", async function (req, res) {
  try {
    ...
    var RegistrationNumber = req.body.txtRegistrationNumber;
    var RegistrationValidity = req.body.txtRegistrationValidity;
    var ChassisNumber = req.body.txtChassisNumber;
    var EngineNumber = req.body.txtEnginenum;
    var OwnerName = req.body.txtowner;
    var NumberOfOwner = req.body.txtNumberofOwner;
    var OwnerDetails = req.body.txtOwnerDetails;
    var ModelName = req.body.txtModelName;
    var ManufactureName = req.body.txtManufactureName;
    var PriceEstimation = req.body.txtPriceEstimation;
    var AgeVehicle = req.body.txtAgeVehicle;
    var clearance = req.body.txtclearance;
    var insurance = req.body.txtInsurance;

    console.log(RegistrationNumber,RegistrationValidity,ChassisNumber,EngineNumber,OwnerName,NumberOfOwner,OwnerDetails,ModelName
      ,ManufactureName,PriceEstimation,AgeVehicle,clearance,insurance)
...
var verifiedDate = new Date();
    ...
    const WriteStatus = await web2toweb3.writeBlockData(
      ClientID,
      SigningPrivateKey,
      UniqueTxID,
      blockJSON,
